...
def run_power_experiment(
    *, output_dir: Path, quick: bool, seed: int
    ...
    _write_csv(output_dir / "resultado_potencias_v2.csv", rows)
    return rows, slopes
...
def run_boundary_experiment(
    *, output_dir: Path, quick: bool, seed: int
    ...
    _write_csv(output_dir / "resultado_saturacion_v2.csv", rows)
    return rows
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(__file__).resolve().parent / "numerical_validation_v2",
    )
    ...
    make_figure(
        output_dir / "persistent_path_validation_v2.png",
        power_rows,
        slopes,
        boundary_rows,
    )
    write_report(
        output_dir / "informe_monte_carlo_v2.txt",
        power_rows,
        slopes,
        boundary_rows,
    )

    print((output_dir / "informe_monte_carlo_v2.txt").read_text(encoding="utf-8"))
